...
export function ProfilePortfolioSections({ profile }: { profile: ProfileDetail }) {
  ...
  const mainSkills = skills.filter((s) => !s.parentSkillId && !s.isCustom)
  const subskills = skills.filter((s) => s.parentSkillId || s.isCustom)

  return (
    <div className="space-y-8">
      {/* Mobile-only Skills & Subskills: appears right after profile card on mobile */}
        ...
        <div className="space-y-4 lg:hidden">
          ...
          {subskills.length > 0 ? (
            <section className="rounded-2xl border border-primary/30 bg-primary/5 p-6 shadow-sm">
              <h3 className="font-heading text-lg font-semibold text-foreground flex items-center gap-2 text-primary">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
        {/* Left Column: About, Projects, Experience */}
        <div className="space-y-8">
          {/* About Section */}
          {hasBio ? (
            <section className="rounded-2xl border border-border/60 bg-card p-6 shadow-sm">
        ...
        {/* Right Column: Core Skills, Subskills (Desktop), Achievements, Certificates */}
        <div className="space-y-8">
          {/* Desktop Core Skills Section */}
          {mainSkills.length > 0 ? (
            <section className="hidden lg:block rounded-2xl border border-border/60 bg-card p-6 shadow-sm">
              <h3 className="font-heading text-lg font-semibold text-foreground flex items-center gap-2">
                <span className="inline-block size-2 rounded-full bg-primary" />
                Core Skills & Expertise
              </h3>
          ...
          {/* Desktop Subskills & Specialized Tags Section */}
          {subskills.length > 0 ? (
            <section className="hidden lg:block rounded-2xl border border-primary/30 bg-primary/5 p-6 shadow-sm">
              <h3 className="font-heading text-lg font-semibold text-foreground flex items-center gap-2 text-primary">
                <Sparkles className="size-5" strokeWidth={1.5} />
                Subskills & Specialized Tags
              </h3>
