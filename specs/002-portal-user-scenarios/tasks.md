...
- [ ] T033 [US2] Create `StatusBadge` component in `src/components/approval/StatusBadge.tsx` — renders amber/pending, green/approved, or red/rejected pill using the soft tag CSS from `docs/design-direction.md` §5 with status colors
- [ ] T034 [US2] Create the "my submissions" page at `src/app/(user)/my-submissions/page.tsx` — shows the user's submitted profile with StatusBadge, link to edit
