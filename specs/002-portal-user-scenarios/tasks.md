...
- [X] T038 [P] [US3] Create `approveItem` Server Action in `src/app/(admin)/approve/actions.ts` — validates caller is admin (or moderator for allowed types), updates status to `approved`, sets approvedBy + approvedAt, inserts notification row for submitter. **Note: Amended post-implementation to use a conditional UPDATE (`WHERE id = ? AND status = 'pending'`) for concurrent-approval safety — 0 rows affected returns "This item was already processed." instead of silently succeeding (CHK026). See 2026-08-02 /speckit.analyze remediation. Admin-entered legacy-alum profiles (`userId = null`) get no notification — no linked account to notify (spec R-016).**
- [X] T039 [P] [US3] Create `rejectItem` Server Action in `src/app/(admin)/approve/actions.ts` — validates caller role, updates status to `rejected`, inserts notification row for submitter with optional reason. **Note: Amended post-implementation to use a conditional UPDATE (`WHERE id = ? AND status = 'pending'`) for concurrent-approval safety — 0 rows affected returns "This item was already processed." instead of silently succeeding (CHK026). See 2026-08-02 /speckit.analyze remediation.**
- [X] T040 [US3] Create unified approval dashboard page at `src/app/(admin)/approve/page.tsx` — shows pending items grouped by resource type (profiles first), filter tabs, pagination; each item shows submitter name, submittedAt, resource type badge, and "Review" button. **Note: implemented as a single `/approve` route serving both admin and moderator roles (role-filtered via `visibleResourceTypes`), since separate `(admin)/approve` and `(moderator)/approve` route groups would collide at the same URL. T042 was merged into this task — the moderator view is the same page whose role-adaptive tabs/queue never surface profile items to moderators, enforced by the role guard + `visibleResourceTypes` (getPendingItems/getPendingCounts take `viewerRole`).**
- [X] T041 [US3] Create approval detail panel component in `src/components/approval/ApprovalCard.tsx` — shows full resource data (for profiles: fullName, studentId, batchNumber, section, avatarUrl, bio, social links, selected skills), "Approve" and "Reject" buttons; on reject, a textarea for the reason
- [X] T042 [US3] Create moderator approval page at `src/app/(moderator)/approve/page.tsx` — same layout as admin approval but filtered to show only moderator-eligible resource types (not profiles). **Note: MERGED INTO T040 — fulfilled by the single `/approve` page whose role-adaptive tabs/queue never surface profile items to moderators; verified via role guard + `visibleResourceTypes`. No separate file required; a separate `(moderator)/approve/page.tsx` would collide with `(admin)/approve/page.tsx` at the same URL. Kept here for history (see 2026-08-02 /speckit.analyze remediation I2).**
- [X] T043 [US3] Create notification query helpers in `src/lib/db/queries/notifications.ts` — `getUnreadCount(userId)`, `getRecentNotifications(userId, limit)`, `markAsRead(notificationId)`, `insertNotification(userId, type, title, message, resourceType, resourceId)`
- [X] T044 [US3] Create notification bell component in `src/components/layout/NotificationBell.tsx` — renders bell icon with unread count badge, dropdown of recent notifications on click, click notification navigates to related page, auto-polls every 30s
- [X] T045 [US3] Integrate notification bell into Navbar component — shows for authenticated users only (depends on T026 Navbar being complete)
- [X] T045b Create notification API route at `src/app/api/notifications/route.ts` — `GET` returns unread count + recent notifications for current user, supports `?markRead={id}` param; used by T044 notification bell polling. **Note: R-016's 30-day auto-clear is DEFERRED — no cleanup job built in Foundation. Intended lightweight approach when built: cleanup-on-read-query (see data-model.md). See 2026-08-02 /speckit.analyze remediation U1.**
...
- [X] T048 [US4] Add rate limit check to `upsertProfile` — enforce 1 profile upsert per hour per user — creation AND edits share one counter, run unconditionally before the create/edit branch; return rate-limit error with `retryAfter` field when exceeded. **Note: creation is no longer exempt — the single 1/hour counter covers both paths (2026-08-02 /speckit.analyze remediation HIGH-1); helper lives in `src/lib/rate-limit.ts` (T049).**
...
- [X] T054 Write a load test that confirms search latency stays under 2s for up to 5,000 profiles (SC-008 scale) — using Playwright or a simple script with EXPLAIN ANALYZE. **Note: scoped to profiles only in Foundation — the 10,000-question half of SC-008 is untestable until the Question Bank exists; question coverage carries to Phase 3 when questions are built (2026-08-02 /speckit.analyze remediation U3).**
