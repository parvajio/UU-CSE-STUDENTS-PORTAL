...
- **Schema file set**: `users`, `profiles`, `skills`, `profile-skills`, `notifications`, `site-config` — all in `src/lib/db/schema/` (relations in `relations.ts`). `notifications` (in-app bell) and `site_config` (generic key/value, e.g. `currentBatch`) were added during Foundation; both documented in `docs/data-dictionary.md`.
