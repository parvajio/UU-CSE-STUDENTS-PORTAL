...
- **Schema file set**: `users`, `profiles`, `skills`, `profile-skills`, `notifications`, `site-config`, `courses` (flat), `questions`, `question-files`, `question-likes`, `question-tags` — all in `src/lib/db/schema/` (relations in `relations.ts`). `subjects` was removed (superseded decision). Question Bank revision 004 introduced multi-file question support (`question_files`), like prevention (`question_likes`), view/download counters, client-side ZIP bundling (`jszip`), and admin course management (`/manage/courses`).
- **`profiles.userId`**: nullable since migration `0003` (additive, not squashed into the initial schema), `unique` when set, `onDelete: CASCADE`. Null for admin-entered legacy-alum profiles, which may pair with a matching `unclaimed` `users` row — see spec §3.3-B (authoritative admin-entry pattern).
- **Rate limiting**: new submission actions use the shared `enforceSubmissionLimit(userId)` wrapper in `src/lib/rate-limit.ts` (5/hour default) rather than reimplementing the check. Profile upsert is the one exception — a combined 1/hour counter for creation AND edits, checked unconditionally at the top of `upsertProfile`. `registerUser` is additionally rate-limited by IP; approve/reject actions are exempt.
- **In-memory rate-limit store limitation**: `checkRateLimit` keeps state in an in-memory `Map` — per-instance and ephemeral on Vercel serverless (approximate under concurrency, resets on cold start). Accepted for MVP at ~200 peak concurrent users; revisit (external store: Upstash/KV or Neon-backed) if concurrent users regularly exceed ~500 or Vercel scales to multiple instances under normal load. Documented in `plan.md`.
- **`career_guidance_requests` is a documented exception to the universal approval pattern**, not a gap — peer-to-peer accept/decline rather than admin-moderated publish-content, so `status/approvedBy/approvedAt` don't apply. Justified in `plan.md`; modeled for Phase 3 (Alumni Network). Do not retrofit the three approval columns onto it.
